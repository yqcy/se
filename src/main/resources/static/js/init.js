//类的全限定名长度超过一定时格式化
function classNameForm(value,row,index){
	if(value.length>53){
...
//异常信息描述长度超过一定时进行格式化
function exceptionForm(value,row,index){
	if(value.length>22){
...
//异常解决方案超过一定时进行格式
function solutionForm(value,row,index){
	if(value.length>22){
		var str = value.substr(0,22);
		return str+"......";
	}else{
...
function stateForm(value,row,index){
...
//格式化《查看异常解决方案的按钮》
function btnForm(value,row,index){
